...
import com.alienvault.report.ReportGenerator;
import com.alienvault.report.Reportable;
import com.alienvault.report.github.GitHubIssueReport;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //Set up the configuration - generally would use Spring for this.
        RepositoryService service = new GitHubService();
        Reportable report = new GitHubIssueReport(service);
        ReportGenerator generator = new ReportGenerator(report);

        try {
            System.out.println(generator.getReport(Arrays.asList(repos)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
